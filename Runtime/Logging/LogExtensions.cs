...
namespace BaseGame
{
    public static class ILogExtensions
    {
        ...
        {
            log.AddEntry(new LogEntry(message, LogEntry.EntryType.Info));
        }
        ...
        public static void LogInfoFormat<L>(this L log, ReadOnlySpan<char> text, ReadOnlySpan<char> arg1) where L : ILog
        {
            ...
            log.AddEntry(new LogEntry(v, LogEntry.EntryType.Info));
        }
        ...
        {
            log.AddEntry(new LogEntry(message, LogEntry.EntryType.Warning));
        }
        ...
        {
            ReadOnlySpan<char> v = ValueStringBuilder.Format(text, arg1);
            log.AddEntry(new LogEntry(v, LogEntry.EntryType.Warning));
        }
        ...
        [HideInCallstack]
        public static void LogError<L, E>(this L log, E exception) where L : ILog where E : Exception
        {
            log.AddEntry(exception);
        }

        [HideInCallstack]
        public static void LogError<L>(this L log, ReadOnlySpan<char> message) where L : ILog
        {
            log.AddEntry(new LogEntry(message, LogEntry.EntryType.Error));
        }
        ...
        {
            ReadOnlySpan<char> v = ValueStringBuilder.Format(text, arg1);
            log.AddEntry(new LogEntry(v, LogEntry.EntryType.Error));
        }
